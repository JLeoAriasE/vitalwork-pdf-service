...
def fill_formulario(data, template_path, output_path, modo='todo'):
    ...
    # Sección N - Retiro (hoja 4)
    r_sec = fo.get('r', {})
    # Se realiza la evaluación: SI/NO - fila 65
    if r_sec.get('se_realiza') == 'SI': ck(ws4, 'AG65', True)
    if r_sec.get('se_realiza') == 'NO': ck(ws4, 'AW65', True)
    # Condición relacionada con trabajo: SI/NO - fila 66
    if r_sec.get('condicion_trabajo') == 'SI': ck(ws4, 'AG66', True)
    if r_sec.get('condicion_trabajo') == 'NO': ck(ws4, 'AW66', True)
    # Observación retiro - filas 68-70
    w(ws4, 'B68', r_sec.get('observacion', ''))
...
if __name__ == '__main__':
    ...
    test_data = {
        ...
        'a': {
            ...
            'grupo_sang': 'O+',
            'lateralidad': 'Diestro',
            'atencion_prioritaria': ['Persona con discapacidad', 'Adulto mayor'],
        },
        'b': {
        ...
        'c': {
            ...
            'gineco': {
                ...
                'examenes': [
                    {'nombre': 'Papanicolau', 'tiempo': '2', 'resultado': 'PACIENTE REFIERE RESULTADO NORMAL'},
                    {'nombre': 'Ecografia mamaria', 'tiempo': '1', 'resultado': 'PACIENTE REFIERE RESULTADO NORMAL'},
                ]
            },
            'masculino': {
                'planif_si': True,
                'planif_cual': 'INYECTABLES (ESPOSA)',
                'examenes': [
                    {'nombre': 'Antigeno prostatico específico', 'tiempo': '2'},
                    {'nombre': 'Eco prostatico', 'tiempo': '1'},
                ]
            },
            'sustancias': [
            ...
            'estilo_vida': {
                'actividad': 'FUTBOL',
                'tiempo': '1 HORA / 2 VECES A LA SEMANA',
            },
            'medicacion': 'LOSARTÁN 50MG QD - METFORMINA 850MG BID',
            'cantidad_med': '1 tableta diaria',
        },
        'd': {
        ...
        'r': {
            'se_realiza': 'SI',
            'condicion_trabajo': 'NO',
            'observacion': 'PACIENTE CON BUENAS CONDICIONES DE SALUD AL MOMENTO DEL RETIRO. SIN PATOLOGÍA RELACIONADA CON EL TRABAJO.',
        },
    }
